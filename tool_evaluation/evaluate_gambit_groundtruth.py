...
# -*- coding: utf-8 -*-
"""
Script to evaluate Gambit algorithm against manual ground truth data.
...
# Add parent directory to path to import gambit
...
def run_gambit_on_aliases(aliases_df):
    """
    Run Gambit algorithm on aliases.
    ...
    """
    # Prepare input for gambit (needs alias_name and alias_email)
    input_df = aliases_df[['alias_name', 'alias_email']].copy()

    try:
        # Run gambit
        result_df = gambit.disambiguate_aliases(input_df, method='gambit')

        # Map alias_id to cluster_id (author_id from gambit)
        alias_to_cluster = {}
        for idx, row in result_df.iterrows():
            alias_id = aliases_df.iloc[idx]['alias_id']
            cluster_id = row['author_id']
            alias_to_cluster[alias_id] = cluster_id

        return alias_to_cluster

    except Exception as e:
        print(f"Error running Gambit: {e}")
        raise
...
def main():
    parser = argparse.ArgumentParser(
        description='Evaluate Gambit algorithm against ground truth data',
...
Examples:
  ...
  python evaluate_gambit_groundtruth.py \\
  ...
  python evaluate_gambit_groundtruth.py \\
    ...
    --output results_gambit_wiese_asf.csv
        """
    )
    ...
    # Run Gambit
    print("\nRunning Gambit algorithm...")
    alias_to_cluster = run_gambit_on_aliases(aliases_df)
    print(f"Gambit created {len(set(alias_to_cluster.values()))} clusters")
