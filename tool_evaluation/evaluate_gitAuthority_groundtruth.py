...
# -*- coding: utf-8 -*-
"""
Script to evaluate GitAuthority algorithm against manual ground truth data.
Supports two ground truth formats:
1. Matrix format (e.g., Gambit data): NxN matrix where non-zero values indicate aliases should be merged
2. Pairwise format (e.g., ALFAA data): Each row has two aliases and a label (1 = merge, 0 = don't merge)
"""
...
# Add parent directory to path to import GitAuthority
...
class GroundTruthLoader:
    ...
    def load_matrix_format(developers_file, groundtruth_file):
        ...
        Returns:
            ...
            - aliases_df: DataFrame with columns [alias_name, alias_email, alias_id]
    ...
    def load_pairwise_format(pairwise_file):
        ...
        Args:
            pairwise_file: CSV file where each row is: alias1;alias2;label;...
        ...
        # Extract alias columns (first two) and label (third or fourth column)
        # Try to find label column (should be 0 or 1)
        label_col = None
        for col_idx in range(2, df.shape[1]):
            if df[col_idx].dtype in [np.int64, np.float64]:
                if set(df[col_idx].unique()).issubset({0, 1, 0.0, 1.0}):
                    label_col = col_idx
                    break

        if label_col is None:
            # Default to column 2 (third column)
            label_col = 2
    ...
    def load_dre_format(dre_file):
        ...
        Returns:
            Tuple of (aliases_df, pairs_list) where:
            - aliases_df: DataFrame with columns [alias_name, alias_email, alias_id, dre_id]
            - pairs_list: List of (id1, id2, should_merge) tuples
        """
        # Read DRE file
    ...
    def load_wiese_format(wiese_file, asf_developers_file=None):
        ...
        Returns:
            Tuple of (aliases_df, pairs_list) where:
            - aliases_df: DataFrame with columns [alias_name, alias_email, alias_id, cluster_id]
            - pairs_list: List of (id1, id2, should_merge) tuples
        """
        import re
    ...
    def load_asf_developers(asf_developers_file):
        ...
        Returns:
            DataFrame with columns [alias_name, alias_email, alias_id]
        """
        import re
        aliases_data = []
        next_id = 0
...
def run_gitauthority_on_aliases(aliases_df, config_path=None):
    """
    Run GitAuthority algorithm on aliases.
    ...
    Returns:
        Dictionary mapping alias_id to cluster_id
    """
    # Prepare input for GitAuthority (needs uid, name, email)
    input_df = aliases_df.copy()
    input_df['uid'] = input_df['alias_id'] + 1  # GitAuthority uses 1-based UIDs
    input_df = input_df.rename(columns={'alias_name': 'name', 'alias_email': 'email'})
    input_df = input_df[['uid', 'name', 'email']]

    try:
        # Run GitAuthority (returns merging dictionary)
        output_dir = "."
        merging_dict = do_merging(input_df, output_dir, config_path)

        # Build cluster mapping
        # merging_dict maps source alias (as JSON) -> target alias (as JSON)
        # We need to build transitive closure to get final clusters

        # First, parse all aliases and build uid -> cluster mapping
        alias_to_cluster = {}

        # Initialize: each alias is in its own cluster
        for idx, row in input_df.iterrows():
            uid = row['uid']
            alias_to_cluster[uid] = uid  # Initially, cluster ID = uid

        # Apply merging (source -> target means source should be in target's cluster)
        # We need to apply this transitively
        for source_json, target_json in merging_dict.items():
            source_data = json.loads(source_json)
            target_data = json.loads(target_json)

            # UIDs in JSON can be strings or ints, convert to int
            source_uid = int(source_data['uid'])
            target_uid = int(target_data['uid'])

            # Merge source into target's cluster
            source_cluster = alias_to_cluster[source_uid]
            target_cluster = alias_to_cluster[target_uid]

            # Update all aliases in source's cluster to target's cluster
            for uid in list(alias_to_cluster.keys()):
                if alias_to_cluster[uid] == source_cluster:
                    alias_to_cluster[uid] = target_cluster

        # Convert from 1-based UIDs back to 0-based alias_ids
        alias_id_to_cluster = {}
        for uid, cluster in alias_to_cluster.items():
            alias_id = uid - 1  # Convert back to 0-based
            alias_id_to_cluster[alias_id] = cluster

        return alias_id_to_cluster

    except Exception as e:
        print(f"Error running GitAuthority: {e}")
        raise
...
def save_error_pairs(pairs_list, output_file, error_type):
    ...
    with open(output_file, 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(['id1', 'id2', 'alias1_name', 'alias1_email', 'alias2_name', 'alias2_email'])
...
def main():
    parser = argparse.ArgumentParser(
        description='Evaluate GitAuthority algorithm against ground truth data',
...
Examples:
  # Matrix format (Gambit data)
  python evaluate_gitAuthority_groundtruth.py \\
  ...
  python evaluate_gitAuthority_groundtruth.py \\
    ...
    parser.add_argument('--config', '-c', type=str, default=None,
                        help='Path to config file (default: merge_config.txt in script directory)')
    ...
    # Run GitAuthority
    print("\nRunning GitAuthority algorithm...")
    alias_to_cluster = run_gitauthority_on_aliases(aliases_df, args.config)
    print(f"GitAuthority created {len(set(alias_to_cluster.values()))} clusters")
